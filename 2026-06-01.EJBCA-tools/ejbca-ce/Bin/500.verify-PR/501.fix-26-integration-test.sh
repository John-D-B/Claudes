#!/usr/bin/env bash
# 501.fix-26-integration-test.sh — exercises the new
...
#   - $certsDir/ELT-Admin.{crt,key} present (written by 214 during the build).
#   - $localDir/ce-target.env present (generated by 221.collect-certs.sh).
#
# Exit 0 on full pass, 1 on any failure.

version='3.2.0'   # 3.2.0 — localDir default out-of-repo (/tmp/claude/demo/local), not ./local.
                  # 3.1.0 — stdlib python via PATH (python3), not ./.venv.
...
# ---------- target config ----------
load_target_config() {
    local cfg="${localDir:-/tmp/claude/demo/local}/ce-target.env"
    if [ ! -f "$cfg" ]; then
