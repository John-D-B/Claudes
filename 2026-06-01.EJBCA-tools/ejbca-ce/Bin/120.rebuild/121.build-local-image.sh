#!/usr/bin/env bash
# 121.build-local-image.sh — build the local ejbca-ce:local-fixes image.
#
# Two steps:
...
#
# Usage:
#   ./Bin/120.rebuild/121.build-local-image.sh             # full build (gradle + docker)
#   ./Bin/120.rebuild/121.build-local-image.sh --skip-ear  # docker step only (reuse last EAR)
#
# Exit 0 on success.
...
==================== 3.3 complete ====================
  ...
  To swap to this image:
    ./Bin/120.rebuild/122.swap-stack-image.sh $IMAGE_TAG

  To roll back to upstream:
    ./Bin/120.rebuild/122.swap-stack-image.sh keyfactor/ejbca-ce:latest

  Integration tests (once swapped):
    ./Bin/140.verify-PR/142.fix-27-integration-test.sh
    ./Bin/140.verify-PR/141.fix-26-integration-test.sh
======================================================
EOF
