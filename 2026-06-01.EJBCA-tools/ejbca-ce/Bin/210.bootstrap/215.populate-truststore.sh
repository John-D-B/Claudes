#!/usr/bin/env bash
# 215.populate-truststore.sh — import ManagementCA into WildFly's truststore.
#
...
echo "==> Waiting for the EJBCA app to redeploy on ${HOST}:8443 (~60s typical)"
# Gate on the AdminWeb returning 200 — that proves the EJBCA *app* is fully
# redeployed, not merely that WildFly's TLS listener is up. Two wrong signals
# to avoid: /ejbcahealth is IP-trust-gated (401 from the host, never ALLOK),
# and a bare /ejbca/ probe is always 404 even when fully deployed — so it can't
# tell "app ready" from "still booting".
for i in $(seq 1 120); do
    code=$(curl -sk -o /dev/null -w '%{http_code}' --max-time 3 \
            https://${HOST}:8443/ejbca/adminweb/ 2>/dev/null || true)
    if [ "$code" = 200 ] || [ "$code" = 302 ]; then
        echo
        echo "  https: AdminWeb up after ~$((i * 2))s"
        echo
        echo "==================== 1.4c complete ===================="
        exit 0
    fi
...
echo
echo "WARNING: https://${HOST}:8443/ejbca/adminweb/ didn't return 200 within ~240s. Check 'docker compose logs ejbca'." >&2
exit 1
