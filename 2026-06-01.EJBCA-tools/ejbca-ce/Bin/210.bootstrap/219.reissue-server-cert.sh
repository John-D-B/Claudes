#!/usr/bin/env bash
# 219.reissue-server-cert.sh — re-issue EJBCA's WildFly server TLS cert
...
echo "      waiting for the AdminWeb on https://${HOST}:8443/ejbca/adminweb/ ..."
for i in $(seq 1 120); do
    code=$(curl -sk -o /dev/null -w '%{http_code}' --max-time 3 https://${HOST}:8443/ejbca/adminweb/ 2>/dev/null || true)
    if [ "$code" = 200 ] || [ "$code" = 302 ]; then
        echo "      back up after ~$((i * 2))s"
        break
    fi
    sleep 2
done
