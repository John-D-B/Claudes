#!/usr/bin/env bash
# 131.smoke-test.sh — automated runner for the v4.0.0 smoke test plan
# defined in Docs/ejbca-ce-task-5.7-smoke-test-plan.md.
#
# Usage:
#   ./Bin/130.probes/131.smoke-test.sh                  # default: --target ce
#   ./Bin/130.probes/131.smoke-test.sh --target ce
#   ./Bin/130.probes/131.smoke-test.sh --target ee
#
...
# Sources ./Bin/elt/<target>-target.env unconditionally if it exists — the
...
load_target_config() {
    local target="$1"
    local cfg="./Bin/elt/${target}-target.env"
    if [ -f "$cfg" ]; then
    ...
    if [ -z "${ELT_HOST:-}" ] || [ -z "${ELT_CERT:-}" ] || [ -z "${ELT_KEY:-}" ]; then
        ...
        if [ "$target" = "ee" ]; then
            echo "       (copy from ./Bin/elt/ee-target.env.example)," >&2
        fi
